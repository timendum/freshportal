import React from "react";
import { type FullFeed } from "../freshrss";

interface AddWidgetProp {
  feeds: FullFeed[];
  open: boolean;
  addWidget: (id: string | null) => void;
  skip: string[];
}

export default function AddWidget({ feeds, open, addWidget, skip }: AddWidgetProp) {
  const ref = React.useRef<HTMLDialogElement>(null);

  React.useEffect(() => {
    const dialog = ref.current;
    if (!dialog) {
      return;
    }
    if (open && !dialog.open) {
      dialog.showModal();
    } else if (!open && dialog.open) {
      dialog.close();
    }
  }, [open]);

  const handleSubmit = (event: React.SyntheticEvent<HTMLFormElement>) => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const id = data.get("feedId");
    if (typeof id !== "string" || !id) {
      return;
    }
    addWidget(id);
  };
  const filteredFeeds = feeds.filter((f) => !skip.includes(f.id)).sort((a, b) => a.title.localeCompare(b.title));
  return (
    <dialog
      ref={ref}
      onClose={() => addWidget(null)}
      onClick={(event) => {
        if (event.target === ref.current) addWidget(null);
      }}
      className="mx-auto mt-20 w-1/2 rounded-xl border border-gray-800 bg-white p-4 shadow-lg backdrop:bg-neutral-400/50 dark:bg-gray-600 dark:shadow-slate-700"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <h4 className="text-lg dark:text-gray-200">Feed to be added:</h4>
        <select name="feedId" className="input-primary block w-full px-3 py-1.5">
          {filteredFeeds.map((feed) => (
            <option key={feed.id} value={feed.id}>
              {feed.title}
            </option>
          ))}
        </select>
        <button
          className="btn-primary w-full bg-blue-800 px-7 py-3 text-sm leading-snug text-slate-200"
          type="submit"
          disabled={filteredFeeds.length < 1}
        >
          Add
        </button>
      </form>
    </dialog>
  );
}
